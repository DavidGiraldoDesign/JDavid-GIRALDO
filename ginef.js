const projectGinef = {
    style: {
        background: 'rgb(255, 252, 250)',
        color: 'rgb(80, 80, 80)'
    },
    coverData: {
        title: 'Ginef ',
        subTitle: '',
        purpose: 'Recommender App for birth control methods',
        youTubeVideoID: null,
        imageCover: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217651/portfolio-ginef/cover-ginef.png'
    },
    briefData: {
        longTitle: 'UX Design applied to recommender apps of birth control methods for young women to enhance reliability of the result.',
        team: 'Jose David Giraldo | Maria Camila Morales',
        scope: 'Academic',
        date: '2017',
        researchTools: 'Personas / Design principle / Usability test / Conducted interview / Information architecture / Recommender algorithms / User centered design / HCI',
        techTools: 'Adobe Photoshop / Adobe illustrator / Javascript / HTML / CSS / Node.js / MongoDB'
    },
    sections: [{
            title: '',
            subTitle: '',
            text: `Ginef is a birth control method recommender app that delivers a more satisfying decision making experience and reliable results. It is tailored for young women having or beginning their sexual relations, and have doubts about witch method is more accurate for their life style and preferences. Furthermore, if the woman has a love partner, she could add his opinion to the recommender system in order to make the decision as a couple. 
            \n The recommendation result is not a medical prescription, it is a guide to help women to be prepared and feel more confidence with their medical appointments. `,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', //logo
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217647/portfolio-ginef/logo-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', //girl holding app
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217654/portfolio-ginef/intro-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: 'Situation',
            subTitle: '',
            text: `In Colombia, the different features of each method offered in Colombia makes it difficult to choose between methods, thus the decision-making process experience is confusing.`,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', //methods in colombia
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217647/portfolio-ginef/methods-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '',
            subTitle: '',
            text: `During a medical appointment, most of the times, the gynecologist only takes into consideration the woman´s biological factors in order to make a medical prescription. That is, the gynecologist may disregard factors related to her life style.`,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: 'Field work',
            subTitle: '',
            text: 'The field work allowed us to understand which factors play into the decision-making process regarding birth control methods. 48 women, between 15 and 24 years old, with an active sexual life, were interviewed.',
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', //Dr Clemencia
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217646/portfolio-ginef/doctor-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: 'Discoveries',
            subTitle: 'What we found',
            text: `70% of the subjects use birth control methods whether they have a sexual partner or not. In addition, the women interviewed expressed concerns about having the boyfriend’s opinion into the decision-making. At the end of the interviews, the variables more relevant in the decision making process were: `,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', //Decision factors
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217645/portfolio-ginef/factors-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: 'User testing ',
            subTitle: '',
            text: ``,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', //UX wirting
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217653/portfolio-ginef/ux-writing-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '',
            subTitle: '',
            text: `Using a content-based filtering approach, we design a functional prototype deployed in a web platform, then we conducted a task analysis to test the usability of the interface, and evaluate the precision of the recommender´s algorithm.`,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', //frist approach
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217646/portfolio-ginef/firts-aproach-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', //girls using the web app
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217655/portfolio-ginef/user-test-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '',
            subTitle: '',
            text: `The result lead us towards a mobile application approach, reducing the amount of information displayed in the interface, and narrowing it down to one question at a time. `,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', // the new approach
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217649/portfolio-ginef/new-aproach-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: 'The design approach',
            subTitle: '',
            text: `Based on the identified variables, the discoveries and the user testing results, we designed a recommender app for birth control method that brings a reliable and satisfying decision making experience for young women during their first approach toward sexual life.`,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', //logo
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217647/portfolio-ginef/logo-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', // Comercial render with flower
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217650/portfolio-ginef/principal-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', // UI Assests
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217645/portfolio-ginef/assests-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: 'Interaction flow',
            subTitle: '1)	Login and registration ',
            text: `The user fills up the form and adds a picture. Afterward, the app clarifies that the results are not a medical prescription, however, they can be used as a guide.`,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', //Login ginef app
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217649/portfolio-ginef/login-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '',
            subTitle: '2)	Gathering information',
            text: `Through a direct method of gathering information, the user answers several questions regarding the variables involved into the decision making process. Each question is answered with “agree” (1), “disagree” (-1) or “Indifferent” (0). `,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', //Ginef diagram with moving methods
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217647/portfolio-ginef/diagram-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '',
            subTitle: '3)	Results, editing and Guide report',
            text: `Ginef discloses the results based on the use´s answers. It organized the methods within a scale that goes from the more convenient to the less convenient. Each of these answers can be edited. 
            \n The result acts as a guide to pursue a more accurate medical perception to the patient´s life style. 
            \n If the woman has a love partner, she can add a second profile, where he can answers the same questions to be taken in consideration. `,
            style: {
                background: '',
                color: ''
            }
        },
        {
            title: '', //Ginef recommender results
            subTitle: '',
            text: ``,
            media: true,
            url: 'https://res.cloudinary.com/dpnemhlg6/image/upload/v1549217649/portfolio-ginef/result-ginef.png',
            youTubeVideoID: null,
            style: {
                background: '',
                color: ''
            }
        }
    ]
}